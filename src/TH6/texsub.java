...
public class texsub extends GLCanvas {
    ...
    private byte subImage [][][] =
    		   new byte [subImageWidth][subImageHeight][4];
	...
	for (i = 0; i < subImageWidth; i++) {
	    for (j = 0; j < subImageHeight; j++) {
	    	if ((((i & 0x4) == 0) ^ ((j & 0x4)) == 0)) c = 255; else  c = 0;
	...
	myGL.glTexCoord2f (1.0f, 1.0f); myGL.glVertex3f (2.41421f, 1.0f, -1.41421f);
	myGL.glTexCoord2f (1.0f, 0.0f); myGL.glVertex3f (2.41421f, -1.0f, -1.41421f);
	myGL.glEnd ();
	myGL.glFlush ();
	...
	switch (key) {
	    ...
	    case 'S':
		myGL.glBindTexture (GL.GL_TEXTURE_2D, texName [0]);
		myGL.glTexSubImage2D (GL.GL_TEXTURE_2D, 0, 33, 22,
				      subImageWidth, subImageHeight , GL.GL_RGBA,
	    ...
	    case 'R':
		myGL.glBindTexture (GL.GL_TEXTURE_2D, texName [0]);
		myGL.glTexImage2D (GL.GL_TEXTURE_2D, 0, GL.GL_RGBA,
